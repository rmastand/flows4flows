...
from utils import get_activation, get_data, get_flow4flow, train, spline_inn, coupling_flow, get_conditional_data, tensor_to_str, \
...
def train_base_flow(*args, **kwargs):
    return train(*args, **kwargs, train_transfer=False)


def train_f4f(*args, **kwargs):
    return train(*args, **kwargs, train_transfer=True)
...
def main(cfg: DictConfig) -> None:
    ...
    #print(get_data(1000).data.dtype)
    ...
    base_data = DataLoader(
        dataset=ScienceDataset(train_sim_data, train_sim_cont),
        batch_size=cfg.base_dist.batch_size,
        shuffle=True
    )
    val_base_data = DataLoader(
        dataset=ScienceDataset(val_sim_data, val_sim_cont),
        batch_size=1000
    )
    
    target_data = DataLoader(
        dataset=ScienceDataset(train_dat_data, train_dat_cont),
        batch_size=cfg.base_dist.batch_size,
        shuffle=True
    )
    val_target_data = DataLoader(
        dataset=ScienceDataset(val_dat_data, val_dat_cont),
        batch_size=1000
    )
    ...
    if pathlib.Path(cfg.base_dist.load_path).is_file():
        ...
        train_base_flow(base_flow, base_data, val_base_data,
    ...
    f4flow = get_flow4flow(cfg.top_transformer.flow4flow,
                           coupling_flow(cfg.general.data_dim,
    ...
    if pathlib.Path(cfg.top_transformer.load_path).is_file():
        ...
        train_f4f(f4flow, target_data, val_target_data,
